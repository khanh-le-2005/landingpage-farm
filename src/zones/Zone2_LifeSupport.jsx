...
export default function Zone2_LifeSupport() {
  const { t } = useTranslation();

  // DANH SÁCH 6 VIDEO YOUTUBE (Bạn có thể thay đổi ID ở đây)
  const videoIds = [
    '5f0ZgR1OTnM', // Video 1
    'StKNK5TB9fU', // Video 2
    'iwxdSxDtVZI', // Video 3
  ...
  return (
    <div className="min-h-full flex flex-col gap-4 p-4 xl:p-6 bg-[#080C14] text-white font-sans overflow-y-auto">
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        ...
        {/* CỘT TRÁI: 6 MÀN HÌNH CAMERA */}
        <div className="flex-[1.1] flex flex-col gap-4">
           <div className="flex justify-between items-center px-2 text-sm font-black uppercase tracking-widest text-green-500">
             RAS AI FISH MONITORING (6-AXIS)
           </div>

           <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[0, 1, 2, 3, 4, 5].map((idx) => (
                <div key={idx} className="flex flex-col gap-1.5">
                  <div className="text-[9px] text-gray-500 font-extrabold px-1 uppercase tracking-wider">
                    {idx === 4 ? 'Clicked Diagnosis' : 'Live Camera'}
                  </div>
                  <div className="border border-white/10 rounded-xl bg-black relative aspect-[4/3] overflow-hidden group shadow-sm">
                    {/* Label AI */}
                    <div className="absolute top-2 left-2 bg-black/80 px-2 py-1 text-[8px] rounded border border-white/20 text-white font-mono z-30">
                      YOLOv11
                    </div>
                    ...
                    <div className="absolute inset-0 z-10">
                      <iframe
                        ...
                        src={`https://www.youtube.com/embed/${videoIds[idx]}?autoplay=1&mute=1&controls=0&loop=1&playlist=${videoIds[idx]}&modestbranding=1&rel=0`}
                        title={`Camera ${idx + 1}`}
                        frameBorder="0"
                        allow="autoplay; encrypted-media"
                      ></iframe>
                      {/* Lớp phủ màn hình camera mờ */}
           ...
           <div className="flex justify-between items-center bg-white/5 border border-white/10 rounded-2xl p-4 mt-1 backdrop-blur-md">
             <ArcGauge title="Velocity" value="0.82" unit="m/s" pct={0.82} color="#00E676" />
             <ArcGauge title="Density" value="15" unit="c/m³" pct={0.6} color="#00E676" />
             <ArcGauge title="Trajectory" value="15" unit="m/s" pct={0.5} color="#FFD600" />
           </div>

           {/* Biểu đồ nhỏ */}
           <div className="grid grid-cols-2 gap-4 mt-1">
              <div className="bg-white/5 border border-white/10 rounded-xl p-2 h-14">
        ...
        {/* CỘT PHẢI: BẢN ĐỒ NHIỆT (THERMAL) */}
        <div className="flex-[0.9] flex flex-col gap-4">
           <div className="flex justify-between items-center px-2">
             <h3 className="text-base xl:text-lg font-black uppercase tracking-widest text-white">THERMAL AI ANALYSIS</h3>
             <div className="text-xs font-bold text-gray-400">NDVI_SCAN</div>
           </div>

           <ThermalHeatMap />

           {/* Thông số bên dưới Heatmap */}
           <div className="p-4 bg-[#0a0a0a] rounded-2xl border border-white/5 shadow-xl flex flex-col gap-3">
              <div className="flex justify-between text-[9px] font-black uppercase tracking-widest">
                <span className="text-red-500">Critical</span>
                <span className="text-green-500">Optimal</span>
              </div>
              <div className="h-1.5 w-full bg-gradient-to-r from-red-600 via-yellow-400 to-green-500 rounded-full border border-white/10" />
              
              <div className="grid grid-cols-2 gap-4 mt-1">
                <div className="flex flex-col">
                  <span className="text-[10px] text-white/40 font-bold uppercase">Biomass Est.</span>
                  <span className="text-2xl font-black text-green-500 italic">14.2<span className="text-[10px] ml-1 uppercase">T/Ha</span></span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] text-white/40 font-bold uppercase">Water Stress</span>
                  <span className="text-2xl font-black text-red-500 italic">0.24</span>
                </div>
              </div>
           </div>
