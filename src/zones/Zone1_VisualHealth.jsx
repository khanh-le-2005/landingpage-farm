...
const MetricCard = ({ children, className = "" }) => (
  <div className={`bg-(--dashboard-bg-item) border border-(--dashboard-stroke) rounded-xl p-3 flex flex-col relative shadow-md hover:border-(--dashboard-stroke-strong) transition-all ${className}`}>
    {children}
  </div>
);
  ...
  return (
    <MetricCard className="items-center justify-between">
      ...
      <div className="relative w-[90px] h-[45px]">
        <svg viewBox="0 0 100 50" className="w-full h-full overflow-visible">
          {/* Base Track */}
          <path d="M 10 50 A 40 40 0 0 1 90 50" fill="none" stroke="currentColor" className="text-(--dashboard-stroke-strong)" strokeWidth="8" strokeLinecap="round" strokeDasharray="2 4" />
          ...
          {/* Ticks */}
          <text x="5" y="55" fontSize="10" fill="currentColor" className="text-(--text-muted)" fontWeight="bold">0</text>
          <text x="45" y="4" fontSize="10" fill="currentColor" className="text-(--text-muted)" fontWeight="bold">40</text>
          <text x="85" y="30" fontSize="10" fill="currentColor" className="text-(--text-muted)" fontWeight="bold">10</text>
        </svg>
        {/* Needle */}
        <motion.div 
          className="absolute bottom-0 left-[50%] w-[3px] h-[36px] bg-(--text-primary) origin-bottom rounded-full z-10 shadow-sm"
          style={{ x: '-50%' }}
          initial={{ rotate: -90 }}
          animate={{ rotate: angle }}
          transition={{ duration: 1.5, type: 'spring' }}
        >
          <div className="absolute top-[-2px] left-[-2px] w-[7px] h-[7px] rounded-full bg-(--text-primary)" />
        </motion.div>
        {/* Center pin */}
...
const SolidBlock = ({ title, statusText, active = true }) => (
  <MetricCard className="justify-between">
    <div className="text-[12px] font-black text-(--text-primary)">{title}</div>
    <div 
      ...
      style={active ? { backgroundColor: 'var(--color-success)' } : { backgroundColor: 'var(--dashboard-stroke)', color: 'var(--text-muted)' }}
    >
      {statusText}
    </div>
  </MetricCard>
...
const BarChartBlock = ({ title }) => (
  <MetricCard className="justify-between">
    ...
    <div className="w-full h-1 mt-1 rounded-full opacity-100" style={{ backgroundColor: 'var(--color-success)' }} />
  </MetricCard>
);
...
const LevelMetric = ({ title, value, unit, pct }) => (
  <MetricCard className="justify-center gap-3">
     <div className="flex justify-between items-center w-full">
       <span className="text-[12px] font-black text-(--text-primary)">{title}</span>
       <DropIcon />
     </div>
     <div className="flex flex-col gap-1 w-full pl-2 pr-6 relative">
        <div className="h-[5px] w-[80%] rounded-full shadow-sm" style={{ backgroundColor: 'var(--color-success)' }} />
        <div className="h-[5px] w-[60%] rounded-full shadow-sm" style={{ backgroundColor: 'var(--color-success)' }} />
        
        {/* Indicator */}
        <div className="absolute top-[-8px] right-[10px] text-center">
           <svg width="10" height="10" viewBox="0 0 24 24" fill="var(--color-success)" stroke="var(--dashboard-bg-item)" strokeWidth="2" className="mx-auto mb-0.5" style={{ fill: 'var(--color-success)' }}>
             <path d="M12 21l-12-18h24z"></path>
           </svg>
           <div className="text-base font-black text-(--text-primary) leading-none mt-1 whitespace-nowrap">{value} <span className="text-[9px] text-(--text-muted) font-black">{unit}</span></div>
        </div>
     </div>
  </MetricCard>
);
...
const IconValueBlock = ({ title, value, Icon }) => (
  <MetricCard className="justify-center items-center gap-2">
     <div className="text-[11px] font-black text-(--text-primary) absolute top-3 left-3 w-full">{title}</div>
...
const GraphValueBlock = ({ title, value, unit, data }) => (
  <MetricCard className="justify-between">
    ...
    <div className="flex items-center gap-2 mt-1">
      <LeafDropIcon />
      <span className="text-2xl font-black text-(--text-primary) tracking-tight">{value}</span>
      <span className="text-[10px] text-(--text-muted) font-black">{unit}</span>
    </div>
    <div className="h-[30px] w-full mt-2">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data}>
          <Line type="monotone" dataKey="val" stroke="var(--color-success)" strokeWidth={3} dot={{ r: 4, fill: 'var(--color-success)' }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  </MetricCard>
...
const SelkartGrid = () => {
  const columns = [
    { height: 0, color: '' },
    { height: 4, color: 'var(--color-success)' },
    { height: 4, color: 'var(--color-success)' },
    { height: 7, color: 'var(--color-gold)' },
    { height: 7, color: 'var(--color-gold)' },
    { height: 4, color: 'var(--color-warning)' },
    { height: 0, color: '' },
    { height: 0, color: '' },
    { height: 0, color: '' },
  ];
  ...
  return (
    <MetricCard className="col-span-1 flex flex-col justify-between p-2 lg:p-3">
      <div className="flex justify-between w-full text-[10px] font-black text-(--text-primary) mb-1">
         <span>Selkart</span>
         <span>Veallap</span>
      </div>
      <div className="flex-1 flex gap-[2px] items-end mt-2 relative pb-[18px] pl-[20px] border-l-2 border-b-2 border-(--dashboard-stroke-strong)">
         {/* Y-axis labels */}
         <div className="absolute left-[-18px] top-0 bottom-4 flex flex-col justify-between text-[8px] text-(--text-muted) font-mono font-bold tracking-tighter">
         ...
         {columns.map((col, idx) => (
            <div key={idx} className="flex-1 flex flex-col-reverse gap-[2px] h-full justify-start">
               {Array.from({ length: 7 }).map((_, rowIdx) => (
                  <div 
                    key={rowIdx} 
                    className={`w-full aspect-square rounded-[2px] ${rowIdx >= col.height ? 'bg-(--dashboard-stroke)' : ''}`} 
         ...
         {/* X-axis labels */}
         <div className="absolute bottom-[-14px] left-[20px] right-0 flex justify-between text-[8px] text-(--text-muted) font-mono font-bold pr-1 tracking-wider">
...
export default function Zone1_VisualHealth() {
  ...
  return (
    <div className="min-h-full flex flex-col p-4 xl:p-6 bg-(--dashboard-bg-deep) text-(--text-primary) font-sans overflow-x-hidden">
      
      {/* HEADER */}
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 px-2 gap-4">
         <h1 className="text-xl xl:text-2xl font-black tracking-widest uppercase drop-shadow-sm border-b-2 pb-1" style={{ color: 'var(--color-gold)', borderColor: 'var(--color-gold)' }}>Camera Live</h1>
         <h2 className="text-2xl xl:text-3xl font-black tracking-widest text-(--text-primary) uppercase drop-shadow-sm text-center">BIOLOGICAL SYSTEM GROUPING</h2>
         <div className="text-base xl:text-lg font-black text-(--text-primary) bg-(--dashboard-bg-card) px-4 py-1.5 rounded-full border border-(--dashboard-stroke) shadow-sm flex items-center gap-2">
            Desert Theme Mode
      ...
      <div className="border border-(--dashboard-stroke-strong) rounded-2xl p-3 xl:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-md mb-4 gap-4 bg-(--dashboard-bg-card)">
         <div className="flex items-center gap-4">
           <div className="p-2.5 rounded-full border border-danger flex items-center justify-center bg-(--dashboard-bg-item)">
        ...
        <div className="flex flex-col gap-4">
           ...
           <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 xl:gap-4 flex-1 auto-rows-fr">
             ...
             <DialGauge title="Positive Pressure" value="15" unit="Dbar" pct={0.8} hasTrack={true} />
              <MetricCard className="justify-between">
                 <div className="text-[12px] font-black text-(--text-primary)">Delta T</div>
                 <div className="flex flex-col justify-end h-full mt-2">
                   <div className="text-4xl font-black flex items-center gap-2" style={{ color: 'var(--color-success)' }}>
                   ...
                   <div className="text-xl font-black text-(--text-primary) mt-2 flex items-center gap-1">
                      <span style={{ color: 'var(--color-success)' }}>»</span> 15 bar
                   </div>
                 </div>
              </MetricCard>
              <MetricCard className="justify-between">
                 <div className="text-[12px] font-black text-(--text-primary)">Montica (µS)</div>
                 <div className="h-[45px] w-full mt-4">
                   <ResponsiveContainer width="100%" height="100%">
                     <LineChart data={trendData}>
                       <Line type="monotone" dataKey="val" strokeWidth={3} dot={{ r: 4, fill: 'var(--color-success)' }} style={{ stroke: 'var(--color-success)' }} />
                     </LineChart>
                   </ResponsiveContainer>
                 </div>
                 <div className="flex justify-between w-full mt-2 px-1">
                   <span className="text-[9px] font-bold text-(--text-muted)">00</span>
                   <span className="text-[9px] font-bold text-(--text-muted)">480</span>
                   <span className="text-[9px] font-bold text-(--text-muted)">1000</span>
                 </div>
              </MetricCard>
           </div>
        </div>
           ...
           <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 xl:gap-4 flex-1 auto-rows-fr">
             ...
             <GraphValueBlock title="Salinity" value="15" unit="ppt" data={salinityData} />
              ...
              <MetricCard className="col-span-1">
                 <div className="text-[12px] font-black text-(--text-muted) tracking-wide mb-1 flex items-center gap-1">
                 ...
                 <div className="text-[13px] font-black text-(--text-primary) leading-tight mb-2 uppercase">Recommendations</div>
                 <div className="text-[10px] text-(--text-muted) whitespace-pre-wrap leading-relaxed border-t border-(--dashboard-stroke) pt-3 font-bold dropdown">
