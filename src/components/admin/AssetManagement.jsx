  ...
  return (
    ...
    <div className="col-span-12 xl:col-span-7 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-3 gap-2">
        {[...Array(6)].map((_, i) => (
          <div key={i} className={`aspect-4/3 bg-black/5 rounded-xl overflow-hidden relative border ${i === 1 || i === 4 ? 'border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.2)]' : 'border-(--dashboard-stroke)'}`}>
            <img 
              src={`/img/fish_cam_${i % 2 === 0 ? '1' : '2'}.png`} 
              alt="Fish Camera" 
              className="w-full h-full object-cover opacity-40 group-hover:opacity-60 transition-opacity duration-700"
            />
            <div className="absolute top-2 left-3 text-[9px] font-bold text-(--text-muted) uppercase tracking-widest">Live Camera</div>
            <div className="absolute top-6 left-3 text-[10px] font-black text-(--text-primary)/90">YOLOV11</div>
            ...
            <div className="flex flex-col items-center">
              <div className="relative w-40 h-24 mb-2">
                <svg className="w-full h-full" viewBox="0 0 100 60">
                  <path d="M10 55 A40 40 0 0 1 90 55" fill="none" stroke="var(--dashboard-stroke)" strokeWidth="6" strokeLinecap="round" />
                  <motion.path
              ...
              <span className="text-[9px] font-black text-(--text-muted)/40 uppercase">vs standard</span>
            </div>
          </Card>
        ))}
      </div>
    ...
    <div className="col-span-12 xl:col-span-5 flex flex-col gap-6">
      <div className="flex items-center justify-between px-2">
        ...
        <span className="text-amber-500 font-extrabold text-2xl">AI Score <span className="text-(--text-primary)">92%</span></span>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-3">
        {[
          ...
          <div key={i} className="flex-1 bg-(--dashboard-bg-item) border border-(--dashboard-stroke) rounded-2xl flex items-center overflow-hidden group">
            <div className="w-32 px-6 flex flex-col shrink-0">
              <span className="text-[10px] font-black text-(--text-muted) uppercase">{item.l}</span>
            ...
            <div className="flex-1 h-full bg-linear-to-r from-black/40 to-transparent relative px-4 flex items-center">
              ...
              <div className="ml-auto flex items-center gap-6 pr-4">
                <div className="flex flex-col items-end">
                  ...
                  <span className="text-[8px] font-bold text-(--text-muted)/30 uppercase tracking-widest leading-none mt-1">Patch W/</span>
                </div>
      ...
      <div className="bg-(--dashboard-bg-item) rounded-4xl p-6 border border-(--dashboard-stroke-strong) relative overflow-hidden">
        <div className="absolute top-2 right-4 text-(--text-muted)">×</div>
        <h4 className="text-[10px] font-black uppercase text-(--text-muted) mb-4 tracking-widest pl-2">Instant Alerts</h4>
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="flex items-center gap-4 px-4 py-3 bg-black/5 rounded-xl border border-(--dashboard-stroke)">
              <div className={`w-3 h-3 rounded-full ${item.c}`} />
              <span className="text-xs font-bold text-(--text-secondary) uppercase">{item.l}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    ...
    <div className="col-span-12 xl:col-span-6 flex flex-col gap-6">
      <div className="flex items-center justify-between px-2">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-(--dashboard-bg-item) rounded-xl border border-(--dashboard-stroke) cursor-pointer">
          <span className="text-sm font-bold text-(--text-primary)">50/50</span>
    ...
    <div className="col-span-12 xl:col-span-6 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card><CardHeader title="Nitrate" /><div className="flex-1 flex items-end justify-center gap-3 px-4 pb-4">{[40, 80, 55, 100, 60].map((h, i) => <div key={i} style={{ height: `${h}%` }} className={`w-5 rounded-t-lg ${i % 2 ? 'bg-green' : 'bg-(--dashboard-stroke)'}`} />)}</div></Card>
        <Card><CardHeader title="EC" subTitle="mS/cm" /><VisualGauge value={2.5} unit="/cm" type="dial" small /></Card>
        <Card><CardHeader title="Drip Nozzle Pressure" /><div className="flex-1 flex items-center justify-center"><div className="relative w-28 h-28 flex items-center justify-center"><svg className="w-full h-full" viewBox="0 0 100 100"><circle cx="50" cy="50" r="45" fill="none" stroke="var(--dashboard-stroke)" strokeWidth="4" /><circle cx="50" cy="50" r="45" fill="none" stroke="#f97316" strokeWidth="6" strokeDasharray="180 300" strokeLinecap="round" /></svg><span className="absolute text-2xl font-black italic text-(--text-primary)">2.0</span></div></div></Card>
        <Card><CardHeader title="Substrate Humidity" /><div className="flex-1 flex items-center justify-center gap-4"><IndicatorIcon type="drop" /><span className="text-4xl font-black italic text-(--text-primary)">60%</span></div></Card>
        <Card><CardHeader title="Internal Humidity" /><div className="flex-1 flex items-center justify-center gap-4"><IndicatorIcon type="target" /><span className="text-4xl font-black italic text-(--text-primary)">70%</span></div></Card>
        <Card><CardHeader title="Leaf Temperature" /><div className="flex-1 flex items-center justify-center gap-6"><div className="h-16 w-6 bg-black/5 rounded-full relative p-1 border border-(--dashboard-stroke)"><div className="absolute inset-x-1 bottom-1 h-1/2 bg-green rounded-full" /></div><span className="text-4xl font-black italic text-(--text-primary)">25 <span className="text-xl opacity-40">°C</span></span></div></Card>
        <div className="flex flex-col gap-4">
          <div className="bg-(--dashboard-bg-item) border border-(--dashboard-stroke) rounded-4xl p-5 h-36 flex flex-col justify-center gap-2">
            <span className="text-[10px] font-black opacity-30 uppercase tracking-widest text-(--text-muted)">Salinity</span>
            <div className="flex items-center gap-3"><div className="w-10 h-10 bg-sky-500/10 rounded-xl flex items-center justify-center text-sky-400 capitalize"><svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L2 7v10l10 5 10-5V7L12 2z" /></svg></div><span className="text-3xl font-black italic text-(--text-primary)">15 <span className="text-xs opacity-30">ppt</span></span></div>
          </div>
          <div className="bg-(--dashboard-bg-item) rounded-2xl p-4 flex items-center gap-3 border border-green/20"><div className="w-2 h-2 rounded-full bg-green" /><span className="text-[9px] font-black uppercase tracking-widest text-(--text-muted)">Medical Indicators</span></div>
        </div>
        <div className="col-span-2 bg-(--dashboard-bg-item) border border-(--dashboard-stroke) rounded-[2.5rem] grid grid-cols-2 p-6 gap-6">
          ...
          <div className="grid grid-cols-10 grid-rows-8 gap-px bg-(--dashboard-stroke) p-px h-full">
            {[...Array(80)].map((_, i) => <div key={i} className={`w-full h-full ${i % 7 === 0 ? 'bg-green/40' : i % 11 === 0 ? 'bg-yellow-500/40' : 'bg-(--dashboard-bg-card)/40'}`} />)}
          </div>
        </div>
      </div>
    </div>
...
const Zone3Dashboard = () => {
  ...
  return (
    ...
    <div className="col-span-1 flex flex-col gap-6">
      <Card className="flex-1 flex flex-col">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-black uppercase tracking-[0.2em] text-(--text-primary)">{t('dashboard.power_balance')}</h2>
          <span className="text-xs font-bold text-(--text-muted) uppercase tracking-widest">(Power Balance)</span>
        </div>
      ...
      <Card>
        ...
        <div className="h-6 w-full bg-black/5 border border-(--dashboard-stroke) rounded-full overflow-hidden relative">
          <motion.div initial={{ width: 0 }} animate={{ width: '85%' }} className="h-full bg-linear-to-r from-green to-emerald-400" />
          <span className="absolute inset-0 flex items-center justify-center text-[10px] font-black text-(--text-primary)">85%</span>
        </div>
      </Card>
      <div className="bg-red-500/5 border border-red-500/20 rounded-[2.5rem] p-8 flex flex-col gap-4">
        <div className="flex items-center gap-4">
          ...
          <h4 className="text-[12px] font-black uppercase tracking-widest text-(--text-primary)">Hệ cảnh báo thông minh</h4>
        </div>
        <p className="text-[10px] font-bold leading-relaxed text-(--text-secondary) uppercase">Hiệu suất Solar giảm &gt;15% so với cùng kỳ bức xạ hôm qua.</p>
        <div className="mt-2 text-[9px] font-black text-orange-500/80 leading-relaxed uppercase">
    ...
    <div className="col-span-1 flex flex-col gap-6">
      <Card className="flex-1 flex flex-col">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-black uppercase tracking-[0.2em] text-(--text-primary)">{t('dashboard.water_gov')}</h2>
          <span className="text-xs font-bold text-(--text-muted) uppercase tracking-widest">(Water Governance)</span>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center pt-10">
          <div className="text-center mb-8">
            <span className="text-[10px] font-black text-(--text-muted) uppercase mb-2 block">Buffer Tank</span>
            <span className="text-5xl font-black italic tracking-tighter text-(--text-primary)">3,000L</span>
          </div>
          <div className="relative w-72 h-44 flex items-center justify-center">
            <svg className="w-full h-full" viewBox="0 0 100 60">
              <path d="M10 55 A40 40 0 0 1 90 55" fill="none" stroke="var(--dashboard-stroke)" strokeWidth="12" strokeLinecap="round" />
              <motion.path
            ...
            <span className="absolute bottom-4 text-4xl font-black italic text-(--text-primary)">75%</span>
          </div>
        </div>
        <div className="px-8 flex flex-col gap-6 pb-10">
          <div className="flex flex-col">
            <span className="text-[11px] font-black uppercase text-(--text-primary)">Khả năng thu hồi (TEC Recovery):</span>
            <span className="text-xs font-bold text-(--text-muted) mt-1 uppercase tracking-widest">Hôm nay (L/h) so với hôm qua (+5%)</span>
          </div>
          <div className="flex flex-col">
            <span className="text-[11px] font-black uppercase text-(--text-primary)">Hiệu quả sourcing (Water Sourcing Ratio):</span>
            <span className="text-xs font-bold text-(--text-muted) mt-1 uppercase tracking-widest">Thu hồi (65%) / Tiêu thụ (100%), RO sử dụng (35%)</span>
          </div>
        </div>
      </Card>
      <div className="bg-(--dashboard-bg-item) border border-(--dashboard-stroke) rounded-[2.5rem] p-8">
        <h4 className="text-center text-[12px] font-black uppercase tracking-widest mb-6 text-(--text-primary)">TEC Task Manager</h4>
        <div className="space-y-4">
          {['Xịt rửa Drum Filter', 'Trung hòa dịch bùn'].map((task, i) => (
            <div key={i} className="flex justify-between items-center text-[10px] font-black uppercase py-3 border-b border-(--dashboard-stroke) last:border-0 text-(--text-secondary)">
              <span>{task}</span>
    ...
    <div className="col-span-1 flex flex-col gap-6">
      <Card className="flex-1 flex flex-col">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-black uppercase tracking-[0.2em] text-(--text-primary)">{t('dashboard.yield_fcr')}</h2>
          <span className="text-xs font-bold text-(--text-muted) uppercase tracking-widest">(Yield & FCR)</span>
        </div>
        <div className="flex-1 flex flex-col gap-4 px-6">
          <div className="flex justify-between items-center">
            <span className="text-[12px] font-black uppercase tracking-widest text-(--text-primary)">Biomass Monitor</span>
            <span className="text-[8px] font-bold text-(--text-muted) uppercase">Plant Density (m)</span>
          </div>
          <div className="flex-1 relative pt-10">
            <svg className="w-full h-full" viewBox="0 0 100 80">
              ...
              {[0, 24, 48, 72, 100].map((x, i) => <circle key={i} cx={x} cy={[60, 65, 60, 55, 20][i]} r="2" fill="var(--text-primary)" />)}
            </svg>
          </div>
          <div className="flex justify-between text-[7px] font-black text-(--text-muted)/40 uppercase tracking-widest mb-4">
            <span>0</span><span>100</span><span>240</span><span>150</span><span>12100</span>
          </div>
          <div className="py-6 border-t border-(--dashboard-stroke) text-[11px] font-black uppercase leading-relaxed text-(--text-muted)">
